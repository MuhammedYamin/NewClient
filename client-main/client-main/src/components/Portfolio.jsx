...
const Portfolio = () => {
  ...
  // Set initial items to show based on zone
  const initialItemsToShow = filter === "All Zones" ? 6 : 3;
  ...
  useEffect(() => {
    filterItems(filter);
    setItemsToShow(filter === "All Zones" ? 6 : 3); // Reset itemsToShow when filter changes
  ...
  const handleLoadMore = () => {
    const maxItemsToShow = filter === "All Zones" ? 6 : 3;

    // If the current itemsToShow is less than 6, increment by the initial number
