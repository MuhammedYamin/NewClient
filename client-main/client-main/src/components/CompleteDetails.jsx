...
const CompleteDetails = () => {
  ...
  const [filter, setFilter] = useState("All Zones"); // Zone filter
  const [filteredProjects, setFilteredProjects] = useState([]); // To store filtered projects
  const [projects, setProjects] = useState([]); // To store all projects
  const [showZoneSelector, setShowZoneSelector] = useState(false);

  const zones = ["Bantwal", "Belthangady", "MangaloreNorth", "MangaloreSouth", "Puttur", "Sullia", "Moodabidre"];
  ...
  const handleZoneSelect = (selectedZone) => {
    ...
    if (selectedZone === "All Zones") {
      ...
      const filtered = projects.filter((project) => project.zone === selectedZone);
  ...
  const getTotalItems = () => {
    if (filter === "All Zones") {
      return projects.length; // Total for all items
    }
    return projects.filter((item) => item.zone === filter).length; // Total for specific zone
  };

  const renderPortfolioItems = () => {
    return filteredProjects.slice(0, itemsToShow).map((item) => (
      <div className="col-lg-4 col-md-6 portfolio-item" key={item._id} data-zone={item.zone}>
  ...
  return (
    <>
      ...
      <div className="successimage">
        <img src={success} alt="Success in Education" className="img-fluid" />
      </div>
      
      <div className="container">
        <div className="section-title">
          <h2 style={{ fontFamily: "cursive", marginBottom: "0px" }}>{filter} Schools</h2>
        </div>

        <hr className="section-divider" />

        <div className="text-center mb-4">
          <blockquote className="mt-3" style={{ fontStyle: "italic", fontSize: "1.2rem" }}>
  ...
  {showZoneSelector && (
    <div className="zone-selector">
      {zones.map((zone) => (
        <button key={zone} className="zone-btn" onClick={() => handleZoneSelect(zone)}>
          {zone}
        </button>
      ))}
      <button className="zone-btn" onClick={() => handleZoneSelect("All Zones")}>
        All Zones
      </button>
    </div>
  )}
...
<style jsx="true">{`
  ...
  .zone-btn {
    ...
    border-radius: 4px;
    text-align: center;
    width: 100%; /* Optional: Makes the buttons full width inside the selector */
  }
`}</style>
